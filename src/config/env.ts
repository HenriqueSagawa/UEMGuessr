  ...
  .superRefine((env, ctx) => {
    if (env.CORS_ORIGIN === "*") {
      ctx.addIssue({
        ...
        path: ["CORS_ORIGIN"],
        message:
          "CORS_ORIGIN não pode ser '*': o browser recusa '*' quando credentials: true está habilitado. Informe a origem exata (ex: https://app.exemplo.com).",
      });
    }
    ...
    if (env.NODE_ENV === "production" && !env.CORS_ORIGIN) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["CORS_ORIGIN"],
        message: "CORS_ORIGIN é obrigatória em produção (o CORS usa credentials: true).",
      });
    }
  });
